...
function EditChapterContent() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const chapter = await apiFetch(`/api/chapters/${id}/`);
        setTitle(chapter.title);
        setContent(Array.isArray(chapter.content) && chapter.content.length > 0
          ? chapter.content
        ...
        setError('Failed to load chapter. You may not have access to edit it.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (error && !content) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <Link
          href="/courses"
          className="mt-4 inline-flex items-center text-sm text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50"
        >
